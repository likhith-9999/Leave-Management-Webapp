...
public class EmployeeRepositoryImpl implements EmployeeRespository {
    ...
    private static final String GET_EMPLOYEE_BY_MAIL_ID_QUERY = "SELECT EMPLOYEE_NAME, EMPLOYEE_PHONE, EMPLOYEE_DOB, "
            + "EMPLOYEE_ID, MANAGER_ID, GENDER FROM EMPLOYEES WHERE EMAIL_ID=?";
    ...
    private static EmployeeRepositoryImpl singleInstance;
    ...
    public static synchronized EmployeeRepositoryImpl getInstance() {
        if (singleInstance == null) {
            singleInstance = new EmployeeRepositoryImpl();
        }
    ...
    public Employee getEmployeeByMail(String emailID) throws SQLException, ClassNotFoundException {
        ...
        while (resultSet.next()) {
            ...
            employee = new Employee(employeeId, employeeName, employeePhone, employeeDOB, emailID,
                    managerId, employeeGender);
        }
        if (employee != null) {
            LOGGER.info("employee was returned");
            return employee;
        }
        LOGGER.warn("No employee found for email ID: {}", emailID);
        return null;
    }

    @Override
    public boolean checkManager(String emailID) throws SQLException, ClassNotFoundException {
        LOGGER.info("Checking if employee with email ID {} is a manager", emailID);
        ...
        LOGGER.warn("No employee found with email ID: {}", emailID);
        return false;
    }
