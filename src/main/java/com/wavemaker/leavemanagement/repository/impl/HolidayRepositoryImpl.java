...
public class HolidayRepositoryImpl implements HolidayRepository {
    ...
    private static final String OVERLAPPING_HOLIDAYS_QUERY = "SELECT FROM_DATE, TO_DATE FROM HOLIDAYS WHERE "
    ...
    private static HolidayRepositoryImpl singleInstance;

    private HolidayRepositoryImpl() {

    }

    public static synchronized HolidayRepositoryImpl getInstance() {
        if (singleInstance == null) {
            singleInstance = new HolidayRepositoryImpl();
        }
        return singleInstance;
    }

    @Override
    public Set<LocalDate[]> getOverlappingHolidays(LocalDate startDate, LocalDate endDate) throws SQLException,
            ClassNotFoundException {
        Connection connection = DataBaseConnection.connect();
        LOGGER.info("getting the holiday between start and end date {} - {}", startDate, endDate);
